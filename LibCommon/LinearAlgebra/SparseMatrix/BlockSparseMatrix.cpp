...
void BlockSparseMatrix<MatrixType>::printDebug() const noexcept
{
    for(UInt i = 0; i < m_Size; ++i) {
        ...
        for(UInt j = 0; j < m_ColIndex[i].size(); ++j) {
            std::cout << m_ColIndex[i][j] << "(" << Formatters::toString(m_ColValue[i][j]) << "), " << std::endl;
        }
        std::cout << std::endl;
    }
        ...
        for(UInt j = 0, jEnd = static_cast<UInt>(m_ColIndex[i].size()); j < jEnd; ++j) {
            for(int l1 = 1; l1 <= 3; ++l1) {
                for(int l2 = 1; l2 <= 3; ++l2) {
                    file << 3 * i + l1 << "    " << 3 * m_ColIndex[i][j] + l2 << "    " << Formatters::toString((*this)(i, j)) << std::endl;
                }
            }
...
template class BlockSparseMatrix<Mat2x2d>;
template class BlockSparseMatrix<Mat3x3d>;
template class FixedBlockSparseMatrix<Mat2x2d>;
template class FixedBlockSparseMatrix<Mat3x3d>;
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
